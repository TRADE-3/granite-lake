...
| Attestation submission | Submitted immediately when connectivity is available (unchanged); queued and auto-retried when it isn't, triggered by the connectivity classifier transitioning to `online`                                                                  |
...
- **`app/`** — a two-step connectivity check (new `connectivity_plus` dependency: radio
  ...
  reports connectivity restored; and history/detail UI showing the on-chain timestamps
  and mode.
- **`server/`** — no changes.
