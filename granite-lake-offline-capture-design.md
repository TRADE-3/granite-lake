...
recorded on its own. Neither one is ever allowed to go missing silently — whenever
either is null, the crew member must supply a reason before the capture is allowed to
proceed, and that reason is hashed and stored so a verifier can later confirm a
disclosed reason matches what was recorded at capture time without the reason text
itself needing to live on a public ledger.
...
- `internet_null_reason_hash` — hash of the mandatory reason given whenever `is_online`
  is `false`, present on both photo and file attestations.
- `has_gps` (photo attestation only) — whether a real GPS fix was used for this capture.
- `is_gps_forced_null` (photo attestation only) — whether the crew manually chose to
  skip GPS despite a fix being available.
- `gps_null_reason_hash` (photo attestation only) — hash of the mandatory reason given
  whenever `has_gps` is `false`.
...
| Mandatory null reason           | Whenever internet or GPS is null at capture time — automatically or via a force toggle — the crew must enter a free-text reason before the shutter proceeds. Required per-field independently: a capture can be missing one, the other, or both, each with its own reason                                                                                                                                                                                                                                                                                        |
